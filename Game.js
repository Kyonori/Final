...
function welcomeScreen(){
	...
	ctx.fillText("DEVELOPED BY: Richard E, Amy Z, Gavin L & Reaz R.", CANVAS_WIDTH/2 , CANVAS_HEIGHT/20*19);
...
function winMessage(){
	...
	ctx.font = "12px monospace";
	ctx.fillText("Press R to replay!", CANVAS_HEIGHT/2, CANVAS_HEIGHT/2+28);
}
...
function loseMessage(){
	...
	ctx.font = "12px monospace";
	ctx.fillText("Replay if you've the guts.Loser!", CANVAS_HEIGHT/2, CANVAS_HEIGHT/2+28);
}
...
function onKeyDown (event) {
	...
	if(!gameOn){
		if(keycode === sCode){
			var music = new Sound("audio.mp3",100,true);
			clearInterval(intervalId);
			gameOn = true;
			gamePaused = false;
			initMaze();
